  ...
  return (
    <div className="mt-4 w-full rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      ...
      {displayCases.length > 1 && (
        <div className="flex items-center gap-1 px-4 py-2 bg-gray-50 border-b overflow-x-auto">
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`text-sm font-bold px-3 py-1 rounded-full ${getStatusBadgeClass(currentCase.status)}`}>
            {currentCase.status}
          </span>
        </div>
        ...
        {currentCase.apiRes?.stderr && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-4">
            <p className="text-xs font-bold text-red-600 uppercase mb-2">Runtime Error</p>
        ...
        {!currentCase.apiRes?.stderr && !currentCase.apiRes?.compile_output && (
          <div className="space-y-4">
            ...
            {currentCase.output && (
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase mb-2">Your Output</p>
                <pre className={`p-4 rounded-lg border font-mono text-sm whitespace-pre-wrap overflow-x-auto ${
                  currentCase.output === currentCase.expected_output
            ...
            {currentCase.apiRes && (
              <div className="grid grid-cols-2 gap-4 text-xs text-gray-500">
                {currentCase.apiRes.time && (
                  <div>
                    <span className="font-medium">Time:</span> {currentCase.apiRes.time}s
                  </div>
                )}
                {currentCase.apiRes.memory && (
                  <div>
                    <span className="font-medium">Memory:</span> {currentCase.apiRes.memory} KB
                  </div>
                )}
              </div>
            )}
...
export default TestCaseOutputSection;