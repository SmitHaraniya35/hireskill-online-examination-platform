...
//       if(res && (res.compile_output || res.stderr )){
//         return;
//       }
    ...
    <div className="bg-white rounded-2xl shadow p-5 flex flex-col h-[600px]">
      <div className="flex justify-between items-center mb-3">
        ...
        <select
          value={language}
          onChange={(e) => setLanguage(e.target.value)}
          className="w-max border border-gray-300 rounded-lg p-2 text-sm"
        >
          {languageOptions.map((lang) => (
            <option key={lang.id} value={lang.id}>
              {lang.name}
      ...
      <div className="flex justify-end gap-3 mt-4">
        ...
        <button
          onClick={onSubmit}
          disabled={isSubmitting || !code.trim()}
          className="px-4 py-2 text-sm rounded-lg bg-[#1DA077] text-white font-semibold hover:bg-[#148562] transition disabled:opacity-50"
        >
          {isSubmitting ? "Submitting..." : "Submit"}
        </button>
      </div>
