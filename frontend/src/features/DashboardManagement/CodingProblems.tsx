  ...
  return (
    <div className="min-h-screen bg-[#f8f9fb]">
      ...
      <div className="w-full px-6 sm:px-10 lg:px-16 xl:px-24 2xl:px-32 py-8 max-w-screen-2xl mx-auto">
        ...
        {/* ── column headers ── */}
        {!loading && filtered.length > 0 && (
          <div className="hidden sm:grid grid-cols-[28px_1fr_90px_140px_64px] gap-x-4 px-4 pb-2.5 text-[10px] font-bold tracking-[0.09em] text-[#c4c9d4] uppercase border-b border-[#f0f1f3]">
            <span />
            <span>Problem</span>
            <span>Difficulty</span>
            <span>Topic</span>
        ...
        {loading ? (
          ...
          <ul className="flex flex-col gap-[3px] mt-1 list-none p-0 m-0">
            {filtered.map((problem, idx) => (
              <li
                key={problem.id}
                className="group grid grid-cols-[28px_1fr_64px] sm:grid-cols-[28px_1fr_90px_140px_64px] gap-x-4 items-center px-4 py-3 rounded-xl bg-[#fafafa] border border-[#eaecef] hover:bg-white hover:border-[#d1d5db] hover:shadow-[0_2px_8px_rgba(0,0,0,0.055)] transition-all duration-150"
              >
                {/* index */}
                <span className="text-[10px] font-bold text-gray-300 text-right select-none tracking-[0.02em]">
                  {String(idx + 1).padStart(2, "0")}
