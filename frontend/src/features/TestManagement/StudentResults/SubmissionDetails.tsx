...
const SubmissionDetails: React.FC<ProblemDetailsProps> = ({ data, loading, error }) => {
  ...
  const renderContent = () => {
    ...
    if (loading) {
      return (
        <div className="flex flex-col items-center justify-center flex-1 gap-3 min-h-56">
    ...
    if (error) {
      return (
        <div className="flex items-center justify-center flex-1 min-h-56">
      ...
      <>
        ...
        <div>
          <div className="flex items-start justify-between gap-3">
            <h1 className="text-base leading-snug">{problem.title}</h1>
            <span
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            <div className="bg-gray-50 border border-gray-100 rounded-xl p-3 flex flex-col gap-1.5">
              ...
              <span
                className={`text-[11px] font-semibold px-2 py-0.5 rounded-md inline-block w-fit mt-0.5 ${statusStyle}`}
              >
                {submission.status}
              </span>
            </div>
  ...
  // Outer shell NEVER unmounts — title stays, only inner content swaps
  return (
    <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 flex flex-col gap-6 overflow-y-auto h-[740px]">

      {/* ── This header is always mounted ── */}
