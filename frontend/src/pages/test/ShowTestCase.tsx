...
const ShowTestCase: React.FC<ShowTestCaseProps> = ({ sampleInput, sampleOutput, testResults }) => {
  const [activeCase, setActiveCase] = useState(0);

  // If no run/submit has happened, show the default sample view
  const displayCases = testResults.length > 0 
    ? testResults 
    : [{ input: sampleInput, expected_output: sampleOutput, status: "Ready" }];
        ...
        {/* {displayCases.map((tc, index) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-xs font-semibold border transition ${
              activeCase === index ? "bg-white border-gray-300 shadow-sm" : "border-transparent text-gray-500"
            }`}
          >
            Case {index + 1}
            <span className={`ml-2 ${
              currentCase.status === "Accepted" ? "text-green-500" : "text-amber-500"
            }`}>●</span>
          </button>
        ))} */}
        {/* // Inside ShowTestCase.tsx mapping the buttons: */}
        {displayCases.map((tc, index) => (
          <button
            key={index}
            onClick={() => setActiveCase(index)}
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`text-sm font-bold px-2 py-0.5 rounded ${
            currentCase.status === "Accepted" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700"
          }`}>
            {currentCase.status}
          </span>
        </div>
        ...
        {currentCase.input && (
          <div className="grid grid-cols-1 gap-4">
            <div>
              ...
              <pre className="mt-1 bg-gray-50 p-2 rounded border font-mono text-sm">{currentCase.input}</pre>
            </div>
            {currentCase.output && (
              <div>
                <p className="text-xs font-bold text-gray-400 uppercase">Your Output</p>
                <pre className="mt-1 bg-gray-50 p-2 rounded border font-mono text-sm">{currentCase.output}</pre>
              </div>
            )}
          </div>
        )}
